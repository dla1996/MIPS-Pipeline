/*
	  __  __ _____ _____   _____   _____ _____ _____  ______ _      _____ _   _ ______    _____ _____ __  __ _    _ _            _______ ____  _____  
 |  \/  |_   _|  __ \ / ____| |  __ \_   _|  __ \|  ____| |    |_   _| \ | |  ____|  / ____|_   _|  \/  | |  | | |        /\|__   __/ __ \|  __ \ 
 | \  / | | | | |__) | (___   | |__) || | | |__) | |__  | |      | | |  \| | |__    | (___   | | | \  / | |  | | |       /  \  | | | |  | | |__) |
 | |\/| | | | |  ___/ \___ \  |  ___/ | | |  ___/|  __| | |      | | | . ` |  __|    \___ \  | | | |\/| | |  | | |      / /\ \ | | | |  | |  _  / 
 | |  | |_| |_| |     ____) | | |    _| |_| |    | |____| |____ _| |_| |\  | |____   ____) |_| |_| |  | | |__| | |____ / ____ \| | | |__| | | \ \ 
 |_|  |_|_____|_|    |_____/  |_|   |_____|_|    |______|______|_____|_| \_|______| |_____/|_____|_|  |_|\____/|______/_/    \_\_|  \____/|_|  \_\
	Denwis La
	PSID::1341755
	ECE 5367
*/
...
public:
	...
	Instructions *next = NULL;	// Holding value for pointing to instruction after
	Instructions *prev = NULL;	// Holding value for pointing to instruction before
private:
...
// Deletes all
void list_Instructions::resetAll()
{
	Instructions *n = top->next;
	while (n != NULL)
	{	
		top->next = n->next;
		n->next = NULL;
...
struct Register
{
	...
	{
		std::fill(R, R + 32, 0);
		usedRegisters.erase(usedRegisters.begin(), usedRegisters.begin() + usedRegisters.size());
	}
};
...
struct Mem
{
	...
	{
		std::fill(M, M + 250, 0);
		usedMemory.erase(usedMemory.begin(), usedMemory.begin() + usedMemory.size());
	}
};
...
bool initFile(const char *fileName, Mem *_memory, Register *_registers, list_Instructions &_List_Instructions)
{
...
void performIF(Instructions &_instruction, Register &_registers, FILE *_fp)
{
	...
	if (std::strtoul(opcode.c_str(), NULL, 2) == 0)
	{
		...
		// Mux
		_instruction.destinationRegister = _instruction.rd;
	...
	else
	{
		...
		_instruction.destinationRegister = _instruction.rt;
...
void performEX(Instructions &_instruction, Register &_registers, Mem &_memory, FILE *_fp)
{
	...
	if (_instruction.function == "ADD")
	{
		...
		_registers.R[_instruction.destinationRegister] = _instruction.newRegisterValue;
	}
	...
	else if (_instruction.function == "SLT")
	{
		...
		{
			_registers.R[_instruction.destinationRegister] = 1;
		}
		else
		{
			_registers.R[_instruction.destinationRegister] = 0;
		}
	}
	else if (_instruction.function == "BEQ")
	{
		...
		if (_registers.R[_instruction.rs] == _registers.R[_instruction.destinationRegister])
		{
	...
	else if (_instruction.function == "SW")
	{
		...
		// Perform Store Word
		_instruction.newRegisterValue = _registers.R[_instruction.destinationRegister];
...
int main()
{
	...
	do
	{
		if ((cont == "y" || cont == "Y") && opened)
		{
			List_Instructions.resetAll();
			List_Instructions.top = NULL;
			registers.reset();
			memory.reset();
			opened = false;
		}

		std::cout << "Please enter the name of your input file with its extension" << std::endl;
		std::cin >> inputFileName;

		std::cout << "Please enter the name of your output file with its extension" << std::endl;
		std::cin >> outputFileName;

		// Get all necessary register, memory, instruction values
		opened = initFile(inputFileName.c_str(), &memory, &registers, List_Instructions);
